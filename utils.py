...
def save_img(image_tensor, filename):
    ...
    Assumes input tensors are already in [0, 1]. If inputs may be in [-1,1],
...
def save_sample_images(inputs, pred, targets, batch_idx, epoch, output_dir):
    ...
    # Pad input to match target spatial size with zeros if needed
    _, h_in, w_in = input_img.shape
    _, h_tg, w_tg = target_img.shape
    pad_h = max(0, h_tg - h_in)
    pad_w = max(0, w_tg - w_in)
    if pad_h > 0 or pad_w > 0:
        input_img = F.pad(input_img, (0, pad_w, 0, pad_h), mode="constant", value=0)
